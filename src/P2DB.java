...
public class P2DB {
    ...
    /**
     * Prompts user for credentials and attempts to establish a connection with the database.
     * 
    ...
    /**
     * Builds and returns a string which can be used to call a stored procedure in the database with parameter(s).
     * 
     * @param packageName   The name of the package which contains the procedure.
     * @param procName      The name of the procedure.
     * @return              The statement that can be executed on the database once parameters are added.
     */
    ...
    /**
     * Builds and returns a string which can be used to call a stored procedure in the database without parameters.
     * 
     * @param packageName   The name of the package which contains the procedure.
     * @param procName      The name of the procedure.
    ...
    public static void showEmployees() throws SQLException {
        ...
        try {                                                                   // and ResultSet
            cs = c.prepareCall(getProcCallWithoutParam(P2Util.PACKAGE_NAME, P2Util.GET_EMPLOYEES)); // Build the procedure call
    ...
    public static void showCustomers() throws SQLException {
        ...
        try {
            cs = c.prepareCall(getProcCallWithoutParam(P2Util.PACKAGE_NAME, P2Util.GET_CUSTOMERS)); // Build the procedure call
            cs.registerOutParameter(1, OracleTypes.CURSOR);                                         // Register a cursor as output
            cs.execute();
            rs = (ResultSet) cs.getObject(1);                                                       // Execute and print the results
            System.out.println("CID" + "\t" + "FIRST NAME" + "\t" + "LAST NAME" + "\t" + "PHONE#" + "\t\t" + "Visits Made#" + "\t" + "Last Visit Date");
            while (rs.next()) {
                ...
                System.out.printf("%s\t%s\t\t%s\t\t%s\t%s\t\t%s%n", cid, firstName, lastName, phone, visitsMade, lastVisitDate);
            }
    ...
    public static void showProducts() throws SQLException {
        ...
        CallableStatement cs = null;                                            // Prepare CallableStatement
        try {                                                                   // and ResultSet
            cs = c.prepareCall(getProcCallWithoutParam(P2Util.PACKAGE_NAME, P2Util.GET_PRODUCTS));  // Build the procedure call
            cs.registerOutParameter(1, OracleTypes.CURSOR);                                         // Register a cursor as output
            cs.execute();                                                                           // Execute and print the results
            rs = (ResultSet) cs.getObject(1);

            System.out.printf("%-10s%-20s%-10s%-15s%-15s%-20s%n", "PID", "NAME", "QOH", "QOH_THRESHOLD", "ORIG_PRICE", "DISCNT_CATEGORY");

            // print table rows
            while (rs.next()) {
                System.out.printf("%-10s%-20s%-10d%-15d%-15.2f%-20.2f%n", rs.getString(1), rs.getString(2), rs.getInt(3), rs.getInt(4), rs.getDouble(5), rs.getDouble(6));
            }
        } catch (SQLException e) {                                              // Catch exceptions
            System.err.println(e.getMessage());
        } catch (Exception e) {
    ...
    public static void showDiscounts() throws SQLException {
        ...
        CallableStatement cs = null;                                            // Prepare CallableStatement
        try {                                                                   // and ResultSet
            cs = c.prepareCall(getProcCallWithoutParam(P2Util.PACKAGE_NAME, P2Util.GET_PROD_DISCNT));   // Build the procedure call
            cs.registerOutParameter(1, OracleTypes.CURSOR);                                             // Register a cursor as output
            cs.execute();                                                                               // Execute and print the results
            rs = (ResultSet) cs.getObject(1);
    ...
    public static void showPurchases() throws SQLException {
        ...
        CallableStatement cs = null;                                            // Prepare CallableStatement
        try {                                                                   // and ResultSet
            cs = c.prepareCall(getProcCallWithoutParam(P2Util.PACKAGE_NAME, P2Util.GET_PURCHASES)); // Build the procedure call
            cs.registerOutParameter(1, OracleTypes.CURSOR);                                         // Register a cursor as output
            cs.execute();                                                                           // Execute and print the results
            rs = (ResultSet) cs.getObject(1);
    ...
    public static void showLogs() throws SQLException {
        ...
        CallableStatement cs = null;                                            // and ResultSet
        try {
            cs = c.prepareCall(getProcCallWithoutParam(P2Util.PACKAGE_NAME, P2Util.GET_LOGS));  // Build the procedure call
            cs.registerOutParameter(1, OracleTypes.CURSOR);                                     // Register a cursor as output
            cs.execute();                                                                       // Execute and print the results
            rs = (ResultSet) cs.getObject(1);
            
            System.out.println(String.format("Logs Details:\n%-6s %-20s %-20s %-15s %-20s %-15s", "log#", "USER_NAME", "OPERATION", "OP_TIME", "TABLE_NAME", "TUPLE_PKEY"));
            while (rs.next()) {
                System.out.println(String.format("%-6s %-20s %-20s %-15s %-20s %-15s", 
                        rs.getString(1), rs.getString(2), rs.getString(3), rs.getDate(4), rs.getString(5), rs.getString(6)));
            }
        } catch (SQLException e) {
            System.err.println(e.getMessage());                                 // Catch exceptions
        } catch (Exception e) {
    ...
    public static void getMonthlySalesReport(String eid) throws SQLException {
        ...
        CallableStatement cs = null;                                            // and ResultSet
        try {
            cs = c.prepareCall(P2DB.getProcCallWithParam(P2Util.PACKAGE_MONTHLY_SALES, P2Util.GET_MONTHLY_SALE));   // Build the procedure call
            cs.registerOutParameter(1, OracleTypes.CURSOR);                                                         // Register a cursor as output
            cs.setString(2, eid);
            cs.execute();                                                                                           // Execute and print the results
            rs = (ResultSet) cs.getObject(1);
    ...
    public static void addEmployee(String eid, String name, String phone, String email) throws SQLException {
        ...
        while(rs1.next()) {
            if (rs1.getString(1).equals(eid)){
                System.out.println("\n\nError: "+ eid + " already exist!!");    // Print an error if employee already exists
                rs1.close();
                return;
            }          
        }
    ...
    public static int verifyStock(String pid) throws SQLException {
        ...
        try {
            ...
            String sql_query = "select QOH from Products where PID = '" + pid + "'";    // Build procedure call to check quantity on hand of product
    ...
    public static void addPurchase(
            ...
            if (currentStock >= qnty) 
            {                                                                                   // If there is enough in stock to complete the purchase,
                cs = c.prepareCall("{call proc_add_purchases.add_purchase(?, ?, ?, ?, ?)}");    // prepare the procedure call to add a new purchase
                cs.setString(1, eid);
                cs.setString(2, pid);
                cs.setString(3, cid);
                cs.setInt(4, qnty);
                ...
                String s1 = "select qoh_threshold from products where PID = '" + pid + "'";     // Retrieve the QOH threshold from the database
                ...
                if(remainingQnty >= qohThreshold){                                              // If the remaining quantity is greater than or equal
                    ...
                    System.out.println("Customer's visit made count updated...");               // and additional updates to database from triggers
                    System.out.println("TRIGGER: Log entries (visits_made & last_visit_date) has been updated for the customer with cid:"+ cid);
                }
                    ...
                    System.out.println("Customer's visit made count updated...");               // and the qoh is updated due to this. Also print details 
                                                                                                // of the updates due to insertion and updates from triggers
                    System.out.println("TRIGGER: Log entries (visits_made & last_visit_date) has been updated for the customer with cid:"+ cid);
                ...
                System.out.println("Insufficient quantity in stock!");  // notify the user that there is insufficient quantity and do not
